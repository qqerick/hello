...
import AssetFieldsForm from "./AssetFieldsForm";
import { showToast } from "../../../components/toastService";
import { useLoader } from "../../../context/LoaderContext";
import muiTheme from "../../../themes/muiTheme";
import { ASSET_FIELDS_QUERY } from "../../../graphql/queries";
import { DELETE_ASSET_TYPE_FIELD_MUTATION } from "../../../graphql/mutations";

const AssetFields = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [assetFieldsData, setAssetFieldsData] = useState<any[]>([]);
  ...
  const [fetchAssetFields, { loading: queryLoading, refetch }] = useLazyQuery(ASSET_FIELDS_QUERY);
  const [hasFetched, setHasFetched] = useState(false);
  const [deleteAssetField] = useMutation(DELETE_ASSET_TYPE_FIELD_MUTATION, {
    refetchQueries: [{ query: ASSET_FIELDS_QUERY, variables: { asset_type_id: null } }],
  });

  const filteredData = useMemo(
    () =>
      assetFieldsData.filter((item) =>
        item.label?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.type?.toLowerCase().includes(searchTerm.toLowerCase())
      ),
    [assetFieldsData, searchTerm]
  );

  // Fetch asset fields data from GraphQL API
  const getAssetFieldsDetails = async () => {
    try {
      showLoader();
      
      // Use refetch if query has been executed before, otherwise use fetchAssetFields
      let queryData, error;
      if (hasFetched && refetch) {
        const result = await refetch({ asset_type_id: null });
        queryData = result.data;
        error = result.error;
      } else {
        const result = await fetchAssetFields({
          variables: { asset_type_id: null },
        });
        queryData = result.data;
        error = result.error;
        setHasFetched(true);
      ...
      const resultData = (queryData as any)?.masterAssetTypeFields;
      
      // Map asset fields to display format
      const normalizedData = Array.isArray(resultData)
        ? resultData.map((item: any) => {
            return {
              id: item.id,
              asset_type_id: item.asset_type_id || "",
              parent_field_id: item.parent_field_id || null,
        ...
        ? [{
            ...
            parent_field_id: resultData.parent_field_id || null,
      ...
      setAssetFieldsData(sortedData);
    } catch (err: any) {
      console.error("Error fetching asset fields:", err);
      setAssetFieldsData([]);
      showToast({
        status: "error",
        title: "Failed to load asset fields.",
      });
    } finally {
      hideLoader();
    }
  ...
  const handleDelete = async (fieldId: string) => {
    try {
      ...
      const deleteResult = (result.data as any)?.deleteMasterAssetTypeField;
      if (deleteResult?.success) {
        showToast({
          status: "success",
          title: deleteResult.message || "Asset field deleted successfully.",
        });
        
        // Refresh table
        getAssetFieldsDetails();
      }
    } catch (error: any) {
      console.error("Error deleting asset field:", error);
      showToast({
        status: "error",
        title: error?.message || "Failed to delete the asset field.",
      });
    } finally {
      hideLoader();
    }
    ...
    <ThemeProvider theme={muiTheme}>
      <Box>
        ...
        <RightSideModal
          ...
          title={modalMode === "add" ? "Add Asset Field" : "Edit Asset Field"}
          key={modalOpen ? `${modalMode}-${selectedField?.id || 'new'}` : undefined}
        >
          <AssetFieldsForm
            mode={modalMode}
            data={selectedField}
            getData={getAssetFieldsDetails}
            onClose={() => setModalOpen(false)}
          />
        </RightSideModal>
      </Box>
